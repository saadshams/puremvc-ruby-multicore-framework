...
class ProxyTest < Minitest::Test
  ...
  def test_data_accessors
    ...
    proxy = Proxy.new('colors')
    proxy.data = %w[red green blue]
    data = proxy.data

    # test assertions
    assert_equal 3, data&.length, 'Expecting data.length == 3' # Method invocation 'length' may produce 'NoMethodError'
  ...
  def test_constructor
    ...
    proxy = Proxy.new('colors', %w[red green blue])
    data = proxy.data
